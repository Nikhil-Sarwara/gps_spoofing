...
The Random Forest's success can be attributed to three factors: (a) its ensemble architecture, which averages predictions across 100 independently trained trees, naturally dampening the variance that drives overfitting; (b) its Gini-based splitting criterion, which makes greedy local decisions that are inherently robust to class imbalance; and (c) the explicit class weighting in scikit-learn's implementation, which directly increases the misclassification cost for minority-class samples.
...
\begin{enumerate}
    ...
    \item \textbf{Small dataset size and class imbalance}. The 76-window dataset is modest by machine learning standards, and the extreme class imbalance (89\% spoofed) creates conditions where models may learn to default to the majority prediction. Larger datasets with balanced representation of normal and spoofed flight regimes, and inclusion of diverse normal flight dynamics (aggressive manoeuvres, wind gusts, sensor dropouts), would enable more robust evaluation.
