...
ARTIFACTS_DIR = _PROJECT_ROOT / "Step_5_Data" / "artifacts"
MODELS_DIR = _PROJECT_ROOT / "Step_5_Data" / "models"
# Default example log
GPS_PROCESSED_CSV = _PROJECT_ROOT / "Step_5_Data" / "processed" / "gps_log_20260402_184048_cleaned.csv"
ROW_LABELS_CSV = _PROJECT_ROOT / "Step_5_Data" / "processed" / "row_labels_auto.csv"
...
def load_artifacts():
    if not (ARTIFACTS_DIR / "dataset.npz").exists():
        raise FileNotFoundError(f"Dataset not found at {ARTIFACTS_DIR / 'dataset.npz'}. Run the pipeline first.")
...
def load_raw_and_labels():
    if not GPS_PROCESSED_CSV.exists():
        console.print(f"[yellow]Warning: Example log {GPS_PROCESSED_CSV.name} not found. Some info will be missing.[/yellow]")
        return None
        
    df_raw = pd.read_csv(GPS_PROCESSED_CSV)
    
    if ROW_LABELS_CSV.exists():
        df_labels = pd.read_csv(ROW_LABELS_CSV)
        # Filter for this specific log if log_name is present
        if "log_name" in df_labels.columns:
            stem = GPS_PROCESSED_CSV.stem.replace("_cleaned", "")
            df_labels = df_labels[df_labels["log_name"].str.contains(stem, na=False)]
...
def run_demo():
    ...
    try:
        ...
        console.print(f"[red]Error loading artifacts: {e}[/red]")
        return
    ...
    with torch.no_grad():
        # X is (N, Window, Features) -> (N, Features, Window) for CNN
    ...
    for i in range(len(X_all)):
        ...
        if len(rows) == 0:
            ...
            mode = rows["mode"].value_counts().index[0] if "mode" in rows.columns else "-"
            failsafe = rows["failsafe"].max() if "failsafe" in rows.columns else "-"
            armed = rows["armed"].max() if "armed" in rows.columns else "-"
        ...
        table.add_row(
            ...
            prob_text,
            str(mode),
            str(int(failsafe) if isinstance(failsafe, (int, float, np.integer, np.floating)) else "-"),
            str(int(armed) if isinstance(armed, (int, float, np.integer, np.floating)) else "-"),
        )
